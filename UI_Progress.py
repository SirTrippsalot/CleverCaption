...
class ProgressBarApp:
    def __init__(self, total_files, total_folders, folder_image_counts):
        ...
        self.current_folder_progress.grid(row=2, column=0, padx=10, pady=10)
        self.current_folder_label = tk.Label(self.root, text="Current Folder: 0/0")
        self.current_folder_label.grid(row=2, column=1, padx=10, pady=10)
        ...
        # Current Folder
        self.current_folder_desc_label = tk.Label(self.root, text="Current Folder")
        self.current_folder_desc_label.grid(row=4, column=0, padx=10, pady=10, sticky="w")
        self.current_folder_name_label = tk.Label(self.root, text="None")
        self.current_folder_name_label.grid(row=5, column=0, padx=10, pady=10, sticky="w")

        # Last Processed File
        self.last_processed_file_desc_label = tk.Label(self.root, text="Last Processed File")
        self.last_processed_file_desc_label.grid(row=4, column=1, padx=10, pady=10, sticky="w")
        self.last_processed_file_label = tk.Label(self.root, text="None")
        self.last_processed_file_label.grid(row=5, column=1, padx=10, pady=10, sticky="w")
    ...
    def update_current_folder_name(self, folder_path):
        ...
        self.current_folder_name_label.config(text=folder_name)

    def update_last_processed_file(self, file_path):
        file_name = os.path.basename(file_path)
        self.last_processed_file_label.config(text=file_name)
    ...
    def update_folders_progress(self, progress_data):
        ...
        max_val = self.current_folder_progress["maximum"]
        self.current_folder_progress["value"] = folders_completed
        self.current_folder_label.config(text=f"Folder Progress: {folders_completed}/{max_val}")
    ...
    def update_current_folder_progress(self, progress_data, folder_image_counts):
        ...
        self.folder_progress["value"] = items_processed_current_folder
        self.folder_progress["maximum"] = max_value
        self.folder_label.config(text=f"Current Folder: {items_processed_current_folder}/{max_value}")
